...
function normalizeEventType(raw?: string) {
    ...
    if (["email_sent", "sent"].includes(v)) return "email_sent";
    if (["opened", "email_opened"].includes(v)) return "opened";
    if (["replied", "email_replied"].includes(v)) return "replied";
    if (["bounced", "email_bounced"].includes(v)) return "bounced";
    if (["unsubscribed", "unsubscribe"].includes(v)) return "unsubscribed";

    return v || "unknown";
}

function computeProviderEventId(payload: any) {
...
function assertAuth(req: VercelRequest) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        try {
            const payload = (typeof (err as any)?.payload !== "undefined" ? (err as any).payload : null) || null;
